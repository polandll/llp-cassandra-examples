...
[.title-ref]#RF=3# can be visualized as follows:

image:images/ring.svg[image]
...
token 2 ([.title-ref]#range(t1, t2]#) are stored on nodes 2, 3 and 4.

=== Multiple Tokens per Physical Node (a.k.a. [.title-ref]#vnodes#)
...
* *Token*: A single position on the [.title-ref]#dynamo# style hash
ring.
* *Endpoint*: A single physical IP and port on the network.
* *Host ID*: A unique identifier for a single "physical" node, usually
present at one [.title-ref]#Endpoint# and containing one or more
[.title-ref]#Tokens#.
* *Virtual Node* (or *vnode*): A [.title-ref]#Token# on the hash ring
owned by the same physical node, one with the same [.title-ref]#Host
ID#.
...
deployments should use the `network-topology-strategy` while the
`simple-strategy` replication strategy is useful only for testing
...
single datacenter, `NetworkTopologyStrategy` should be preferred over
...
the `Snitch <snitch>`. If the number of racks is greater than or equal
...
Transient replication is an experimental feature in Cassandra 4.0 not
...
read_repair set to `NONE` and monotonic reads are not currently
supported. You also can't use `LWT`, logged batches, or counters in 4.0.
...
Transient replication is an experimental feature that may not be ready
...
It is anticipated that `4.next` will support monotonic reads with
...
[.title-ref]#Replica read repair <read-repair># in the read path and
[.title-ref]#Hinted handoff <hints># in the write path.

These techniques are only best-effort, however, and to guarantee
eventual consistency Cassandra implements [.title-ref]#anti-entropy
