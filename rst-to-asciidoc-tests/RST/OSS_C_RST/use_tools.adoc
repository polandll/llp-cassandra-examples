...
advanced tool such as `bcc-tools <use-bcc-tools>`.
...
use advanced tools like `cachestat <use-bcc-tools>` or
`vmtouch <use-vmtouch>` to dive deeper.
...
missing the cache (per e.g. `cachestat <use-bcc-tools>`), in which case
