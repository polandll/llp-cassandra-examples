...
meet emerging large scale, both in data footprint and query volume,
...
Cassandra provides the Cassandra Query Language (CQL), an SQL-like
...
`fqltool` are new tools in Apache Cassandra 4.0.
...
Apache Cassandra 4.0 has added several new features including virtual
tables. transient replication, audit logging, full query logging, and
support for Java 11. Two of these features are experimental: transient
